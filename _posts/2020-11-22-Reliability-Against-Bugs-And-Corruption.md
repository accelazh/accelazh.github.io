...
Compared to end-to-end verification, we can name the __chain-of-trust verification__ concept. In typical verification, every step of data transform needs to be verified, e.g. memory copy, message sending, writing to disk, transforming to other formats, calculating dependent results, etc.  Memory corruption is not uncommon to see on massive production scale.
  ...
  * The operation on CRC also needs to be verified. This is because CRC will usually be memorized in data blocks and metadata, as the source to determine whether the data is "correct". If the CRC is memorized wrong, it breaks data health management.  Failure patterns can be, CRC is copied in memory but bit flip, CRC is written to disk but bit flip, CRC can be concatenated or reused (to calculate CRC on longer data) but yields an incorrect result.  Typical solutions can be, compute twice and compare, wait on disk scrubbing done before declare finish healthily.
  ...
  * Chain-of-trust may need not be enough. A=>B then verify A has problem. This is because even A is correct (memory correct), CPU computation can still inject corruption. Then the choices we have are: 1) Compute A=>B twice, on a different copy of source memory, on a different node/CPU, or with a heterogeneous algorithm. 2) A<=B revertible, e.g. encode A=>B, decode B=>A.
